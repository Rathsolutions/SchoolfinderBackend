...
public class PersonController {
    private static final String THIS_PERSON_COULD_NOT_BE_FOUND_PLEASE_SPECIFY_MORE_PARAMETERS
            = "This person could not be found, please specify more parameters!";
    ...
    public ResponseEntity<Boolean> existsPerson(String prename, String lastname, String email,
            String phonenumber) {
        return ResponseEntity.ok(personRepo.existsByPrenameAndLastnameAndEmailAndPhoneNumber(
            prename, lastname, email, phonenumber));
    }
    ...
    public ResponseEntity<?> getPerson(String prename, String lastname, String email) {
        if (prename.isBlank() && lastname.isBlank() && email.isBlank()
            ...
            return ResponseEntity.badRequest()
                    .body(THIS_PERSON_COULD_NOT_BE_FOUND_PLEASE_SPECIFY_MORE_PARAMETERS);
        }
        Optional<Person> person = null;
        if (!email.isBlank() && personRepo.countByEmail(email) == 1) {
            person = personRepo.findByEmail(email);
        } else if (email.isBlank()
                && personRepo.countByPrenameAndLastname(prename, lastname) == 1) {
    ...
    public ResponseEntity<List<Person>> getEmailRecommendations(String prename, String lastname,
            String email, String amount) {
        PageRequest page = PageRequest.of(0, Integer.valueOf(amount));
        return ResponseEntity.ok(
            personRepo.findByPrenameAndLastnameAndEmailContaining(prename, lastname, email, page));
    }
    ...
    public ResponseEntity<Person> addNewPerson(
            @RequestBody AddNewPersonPostbody addNewPersonPostbody) {
        if (personRepo.findByPrenameAndLastnameAndEmailAndPhoneNumber(
            addNewPersonPostbody.getPrename(), addNewPersonPostbody.getLastname(),
            addNewPersonPostbody.getEmail(), addNewPersonPostbody.getPhoneNumber()).isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).build();
        }
        Optional<Person> personByEmail = personRepo.findByEmail(addNewPersonPostbody.getEmail());
        if (personByEmail.isPresent()
                && (personByEmail.get().getPhoneNumber()
                        .equals(addNewPersonPostbody.getPhoneNumber())
                        || personByEmail.get().getEmail().equals(addNewPersonPostbody.getEmail()))
                && (!personByEmail.get().getPrename()
                        .equalsIgnoreCase(addNewPersonPostbody.getPrename())
                        || !personByEmail.get().getLastname()
                                .equalsIgnoreCase(addNewPersonPostbody.getLastname()))) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }
        Person person
                = new Person(addNewPersonPostbody.getPrename(), addNewPersonPostbody.getLastname(),
